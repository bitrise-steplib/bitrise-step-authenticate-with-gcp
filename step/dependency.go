...
func (s *Step) InstallDependencies() error {
	...
	version, isInstalled, err := s.installedCLIVersion()
	if err != nil {
		return s.installCLI()
	}

	if isInstalled && version == supportedCLIVersion {
...
func (s *Step) installCLI() error {
	...
	tarPath, err := downloadSDK(url, s.logger)
	if err != nil {
...
func downloadSDK(url string, logger log.Logger) (string, error) {
	...
	resp, err := retryhttp.NewClient(logger).Get(url)
	if err != nil {
		return "", err
	}
